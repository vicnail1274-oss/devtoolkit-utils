/**
 * Hash a string using the specified algorithm via Web Crypto API.
 ...
 * Note: MD5 is NOT cryptographically secure. Use SHA-256+ for security purposes.
 */
export function md5(input: string): string {
  ...
  for (i = 64; i <= n; i += 64) {
    ...
    for (let j = i - 64; j < i; j += 4) {
      block.push(
        input.charCodeAt(j) | (input.charCodeAt(j + 1) << 8) |
        (input.charCodeAt(j + 2) << 16) | (input.charCodeAt(j + 3) << 24)
      );
    }
  ...
  for (let j = 0; j < n % 64; j++) {
    tail[j >> 2] |= input.charCodeAt(i - 64 + j + (i > 64 ? 64 : 0)) << ((j % 4) << 3);
  }
  // Actually let me simplify - use the remaining bytes properly
  const rem = n % 64;
  for (let j = 0; j < 16; j++) tail[j] = 0;
  const offset = n - rem;
  for (let j = 0; j < rem; j++) {
    tail[j >> 2] |= input.charCodeAt(offset + j) << ((j % 4) << 3);
  }
